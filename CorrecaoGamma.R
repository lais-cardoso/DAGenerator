...
correcaoGammaFilter = function(nImageCorrecaoGamma){
  remove(list=ls())
  ...
  for(i in 1:length(nomes)){
    ...
    png(paste("transform/correcaoGamma/originalCorrecaoGamma",randomValue,nomes[i],im,".png"))
  ...
  #for com 1 numero a mais que o desejado
  for(j in 1:nImageCorrecaoGamma){
    for(i in 1:length(nomes)){
      im=readImage(nomes[i])
      im2 = resize(im, 500) #6 MB
      plot(im2)
      ...
      png(paste("transform/correcaoGamma/correcaoGamma",randomValue,nomes[i],im,".png"))
